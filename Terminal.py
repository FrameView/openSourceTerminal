...
from PyQt6.QtGui import QPixmap, QPalette, QTextCursor, QFont, QColor
from PyQt6.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QPushButton, QLabel, QFileDialog, QMessageBox, QHBoxLayout, QTextEdit, QLineEdit, QColorDialog
...
#Buttons
        ...
        self.background_button = QPushButton("🖼️")
...
#Connect signals        
        ...
        self.color_button.clicked.connect(self.switch_text_color)
...
#Toggle Background
    def switch_background(self):
        self.file_path, _ = QFileDialog.getOpenFileName(self, "Select an image", "", "Image Files (*.png *.jpg *.jpeg *.bmp *.gif);;All Files (*)")
        ...
        try:
            ...
            self.container.setStyleSheet(f"""
                QWidget {{
                    background-image: url({self.file_path});
...
#Toggle Text Color
    def switch_text_color(self):
        color = QColorDialog.getColor( 
            initial=QColor("#"+self.defaultColor),  
            parent=self,
            title="Select Color",
            options=QColorDialog.ColorDialogOption.DontUseNativeDialog | QColorDialog.ColorDialogOption.ShowAlphaChannel)
        
        if color.isValid():                                  
            hex_color = color.name(QColor.NameFormat.HexRgb) 
            self.defaultColor = hex_color[1:]                
            self.container.setStyleSheet(f"""      
                QWidget {{
                    background-image: url({self.file_path});
                    background-position: center;
...
#Errors handler
    def handle_stderr(self):
        try:
            ...
            self.terminal_output.append("<span style='color: #{};'></span>".format(self.defaultColor))  
            self.scroll_to_bottom()
        except Exception as e:
